...
function Information() {
    ...
    const setUserStyle = (action, state) => {
        if (action === 'input') return state ? 'inline-block' : 'none';
        else return state ? 'none' : 'inline-block';
    };

    const setPasswordStyle = (action, state) => {
        if (action === 'close') return state ? 'inline-block' : 'none';
        if (action === 'li') return state ? 'none' : 'block';
        if (action === 'input') return state ? 'block' : 'none';
        else return state ? 'none' : 'inline-block';
    };
            ...
            <div className="setting-content-info-container">
                ...
                <ul className="setting-info-list">
                    <li className="setting-info-item">
                        ...
                        <input
                            type="text"
                            className="setting-update-input"
                            style={{ display: setUserStyle('input', showUser) }}
                            value={updateUser}
                            onChange={(e) => setUpdateUser(e.target.value)}
                        />
                        <p className="setting-user-infomartion" style={{ display: setUserStyle('p', showUser) }}>
                            {user.username}
                        </p>
                        <p
                            className="setting-userinfo-btn"
                            onClick={() => setShowUser(true)}
                            style={{ display: setUserStyle('p', showUser) }}
                        >
                            update
                        </p>
                        <p
                    ...
                    <li className="setting-info-item" style={{ display: setPasswordStyle('li', showPassword) }}>
                        ...
                        <p
                            ...
                            style={{ display: setPasswordStyle('p', showPassword) }}
                        >
                            update
                        </p>
                    </li>
                    <li className="setting-info-item" style={{ display: setPasswordStyle('input', showPassword) }}>
                        ...
                        <p
                            ...
                            style={{ display: setPasswordStyle('close', showPassword) }}
                        >
                            save
                        </p>
                    </li>
                    ...
                    <li className="setting-info-item">
                        ...
                        <input
                            type="text"
                            className="setting-update-input"
                            style={{ display: setUserStyle('input', showEmail) }}
                            value={updateEmail}
                            onChange={(e) => setUpdateEmail(e.target.value)}
                        />
                        <p className="setting-user-infomartion" style={{ display: setUserStyle('p', showEmail) }}>
                            {user.email}
                        </p>
                        <p
                            className="setting-userinfo-btn"
                            onClick={() => setShowEmail(true)}
                            style={{ display: setUserStyle('p', showEmail) }}
                        >
                            update
                        </p>
                        <p
                    ...
                    <li className="setting-info-item">
                        ...
                        <input
                            type="text"
                            className="setting-update-input"
                            style={{ display: setUserStyle('input', showPhone) }}
                            value={updatePhone}
                            onChange={(e) => setUpdatePhone(e.target.value)}
                        />
                        <p className="setting-user-infomartion" style={{ display: setUserStyle('p', showPhone) }}>
                            {user.phone}
                        </p>
                        <p
                            className="setting-userinfo-btn"
                            onClick={() => setShowPhone(true)}
                            style={{ display: setUserStyle('p', showPhone) }}
                        >
                            update
                        </p>
                        <p
